 ...
 *
 * APIs used here all support CORS for public browser requests:
  ...
  await Promise.allSettled(
    GREENHOUSE_IDS.map(async (id) => {
      ...
      for (const item of data.jobs ?? []) {
        ...
        if (req.location && !loc.toLowerCase().includes(req.location.toLowerCase()) && wtype !== "remote") continue;
        jobs.push({
...
async function searchLever(req: SearchRequest): Promise<Job[]> {
  ...
  await Promise.allSettled(
    LEVER_IDS.map(async (id) => {
      const r = await fetch(
        `https://api.lever.co/v0/postings/${id}?mode=json`
      );
      if (!r.ok) return;
      const data = await r.json();
  ...
  await Promise.allSettled(
    ASHBY_IDS.map(async (id) => {
      ...
      for (const item of data.jobPostings ?? []) {
        const title: string = item.title ?? "";
        if (!title.toLowerCase().includes(q) && !JSON.stringify(item).toLowerCase().includes(q)) continue;
        ...
        if (req.work_type !== "any" && wtype !== req.work_type) continue;
        if (req.location && !loc.toLowerCase().includes(req.location.toLowerCase()) && !isRemote) continue;
...
async function searchRemotive(req: SearchRequest): Promise<Job[]> {
  ...
  const r = await fetch(
    `https://remotive.com/api/remote-jobs?search=${encodeURIComponent(req.query)}&limit=20`
  );
  if (!r.ok) return [];
  const data = await r.json();
